...
export class SignatureService {
  ...
  private savedSignatures: SignatureData[] = [];

  saveSignature(signature: SignatureData): void {
    this.savedSignatures.push(signature);
  }

  getSavedSignatures(): SignatureData[] {
    return [...this.savedSignatures];
  }

  deleteSignature(hash: string): boolean {
    const index = this.savedSignatures.findIndex(sig => sig.hash === hash);
    if (index !== -1) {
